...
from .performance import calculate_climb_rate
...
class ChartDataGenerator:
    ...
    def get_lift_curve_data(self, ac: SizedAircraft, alpha_range_deg: List[float] = None) -> Dict[str, Any]:
    ...
    def get_drag_polar_data(self, ac: SizedAircraft, cl_range: List[float] = None) -> Dict[str, Any]:
        if cl_range is None:
    ...
    def get_climb_performance_data(self, ac: SizedAircraft) -> Dict[str, Any]:
        ...
        e = ac.geometry.get('oswald_e', 0.8)
        ar = ac.geometry.get('aspect_ratio', 8.0)
        polar = AeroPolar(cd0=cd0, e=e, aspect_ratio=ar)
