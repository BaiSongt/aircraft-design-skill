...
class StaticPlotter:
    ...
    def plot_3view(self, geom: Dict[str, Any]) -> Dict[str, str]:
        ...
        def get_rect_wireframe(l, w, x, y, z):
        ...
        def get_fus_wireframe(l, d):
            # Cylinder approx
            xs = [0, l, l, 0, 0]
            ys = [d/2, d/2, -d/2, -d/2, d/2]
            return xs, ys
        ...
        ax.add_patch(plt.Rectangle((fx, fy - fd/2), fl, fd, fill=False, edgecolor='black', linewidth=2, label='Fuselage'))
        ...
        rw_pts = [
            [wx, wy],
            [wx + tip_x_off, wy + span],
            [wx + tip_x_off + wct, wy + span],
            [wx + wcr, wy]
        ]
